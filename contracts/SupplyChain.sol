...
/* Create a modifer that checks if the msg.sender is the owner of the contract */
  ...
  modifier forSale(uint sku){
    require(items[sku].state == State.ForSale && items[sku].sku == sku);
    _;
  }
  modifier sold(uint sku){
    require(items[sku].state == State.Sold && items[sku].sku == sku);
    _;
  }
  modifier shipped(uint sku){
    require(items[sku].state == State.Shipped && items[sku].sku == sku);
    _;
  }
  modifier received(uint sku){
    require(items[sku].state == State.Received && items[sku].sku == sku);
    _;
  }
